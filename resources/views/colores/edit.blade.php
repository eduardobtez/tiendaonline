...
@section('content')
<h1 class="h3 mb-3">Editar Color</h1>
...
<form action="{{ route('colores.update', $color) }}" method="POST">
    ...
    <div class="mb-3">
        ...
        <input type="text" name="nombre" class="form-control" value="{{ old('nombre', $color->nombre) }}" required>
        @error('nombre') <div class="text-danger small">{{ $message }}</div> @enderror
    </div>

    <div class="mb-3">
        <label for="codigo_hex" class="form-label">Código Hex</label>
        <input type="text" name="codigo_hex" class="form-control" value="{{ old('codigo_hex', $color->codigo_hex) }}">
        @error('codigo_hex') <div class="text-danger small">{{ $message }}</div> @enderror
    </div>

    <div class="mb-3">
        <label for="imagen_url" class="form-label">URL Imagen</label>
        <input type="url" name="imagen_url" class="form-control" value="{{ old('imagen_url', $color->imagen_url) }}">
